...
int main(int argc, char const *argv[])
{
	...
	dev::test::IsolOptions options(&TestTool::editor);
	...
	for (auto const& ts: g_interactiveTestsuites)
	{
		...
		if (auto stats = runTestSuite(ts.title, options.testPath / ts.path, ts.subpath, options.ipcPath.c_str(), ts.testCaseCreator, options.formatted))
			global_stats += *stats;
		else
			return 1;
	}
